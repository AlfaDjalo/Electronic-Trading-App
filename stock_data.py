...
class StockData:
    def __init__(self, ticker, start_date=DEFAULT_START_DATE, end_date=DEFAULT_END_DATE, split_date='2022-12-31', load_data=False, create_model_data=False):
        """
    ...
    # def merge_lob_with_stock_data(self):
    #     """
    #     Merge LOB data with stock data based on the timestamp.

    #     Raises:
    #         ValueError: If LOB data or stock data is not loaded.
    #     """
    #     if not hasattr(self, 'lob_data'):
    #         raise ValueError("LOB data is not loaded. Use load_lob_data() first.")

    #     if self.data is None:
    #         raise ValueError("Stock data is not loaded. Use load_data() first.")

    #     # Merge LOB data with stock data on the index (datetime)
    #     self.data = self.data.merge(self.lob_data, how='left', left_index=True, right_index=True)
    #     print("LOB data merged with stock data.")
